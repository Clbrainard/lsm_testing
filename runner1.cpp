...
std::vector<double> generatePricePathMatrix(
    ...
    std::vector<double> paths((size_t)P * N);
    ...
    // When called from within an outer parallel region (multiple simulations running
    // concurrently), limit inner threads to 1 to avoid oversubscribing the CPU.
...
std::vector<long double> priceAmericanPut(
    ...
    for (int batchStart = 0; batchStart < P; batchStart += batch_size) {
        ...
        unsigned int batchSeed = seed + (unsigned int)(batchStart / batch_size) * 1000003u;
        std::vector<double> S = generatePricePathMatrix(bP, So, dt, N, r, v, batchSeed);
...
int main() {
    ...
    */
    ...
    std::vector<int> Ns = {1,2,3,4,5,6,7,9,10};

    int regType = 1;

    int numTests = 100 * (int)cases.size();
    std::atomic<int> complete(0);

    // Detect physical RAM and cap concurrent simulations to avoid OOM.
    // Each simulation's peak allocation is one path batch (mirrors the batch logic
    // in priceAmericanPut, using the largest N as the worst case).
#ifdef __unix__
    long phys_pages = sysconf(_SC_PHYS_PAGES);
    long page_sz    = sysconf(_SC_PAGE_SIZE);
    size_t total_ram = (phys_pages > 0 && page_sz > 0)
                       ? (size_t)phys_pages * (size_t)page_sz
                       : (size_t)16ULL * 1024 * 1024 * 1024;
#else
    size_t total_ram = (size_t)16ULL * 1024 * 1024 * 1024;
#endif
    {
        int maxN = *std::max_element(Ns.begin(), Ns.end());
        const size_t BATCH_CAP = (size_t)4 * 1024 * 1024 * 1024;
        size_t bpp = (size_t)2 * maxN * sizeof(double);
        size_t bsz = ((size_t)P * bpp > BATCH_CAP) ? BATCH_CAP / bpp : (size_t)P;
        if (bsz < 1) bsz = 1;
        size_t mem_per_sim = bsz * (size_t)maxN * sizeof(double);
        int safe = (int)((total_ram * 0.75) / (double)mem_per_sim);
        int sim_threads = std::max(1, std::min(safe, omp_get_max_threads()));
        omp_set_num_threads(sim_threads);
        std::cout << "RAM: " << (total_ram >> 30) << " GB"
                  << "  mem/sim: " << (mem_per_sim >> 20) << " MB"
                  << "  concurrent simulations: " << sim_threads << "\n";
    }
    ...
    for (int z = 0; z < (int)cases.size(); z++) {
        ...
        // Each of the 100 runs is an independent Monte Carlo estimator (different seed),
        // so they can safely execute in parallel across cores.
        #pragma omp parallel for schedule(static)
        for (int i = 0; i < 100; i++) {
            int c = ++complete;
            {
                std::lock_guard<std::mutex> lock(io_mutex);
                std::cout << "Running simulations: " << c << "/" << numTests <<"\n";
            }
